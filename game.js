...
Player.prototype.draw = function () {
	this.drawAllBullets();
	ctxEntities.drawImage(imgSprite, this.srcX, this.srcY, this.width, this.height, this.drawX, this.drawY, this.width, this.height, this.dWidth, this.dHeight);
};
...
Player.prototype.checkTouchingPotion = function (newDrawX, newDrawY) {
	...
	if (potionX < newCenterX
		...
		&& newCenterY < (potionY + potionHeight) - 20) {
		} else {
			return false
		}
};
...
function defineObstacles() {
	var treeWidth = 65,
		...
		obstacles = [new Obstacle(78, 360, treeWidth, treeHeight),
			...
			new Obstacle(412, 102, treeWidth, treeHeight),
...
Potion.prototype.draw = function () {
	if(!this.pickedUp) {
		ctxEntites.drawImage(potionSprite, this.drawX, this.drawY)
	} else {
		this.drawX = -1;
		this.drawY = -1;
		this.pickedUp = true;
